...
- brand page：抽取 2026 年卡片（detail_url / size_inch / price_cny / release_text）
...
运行（Git Bash）：
  python step3_batch_brand_paths_2026.py --brands_yaml brands.yaml --target_year 2026 --out_root out_step3_2026 --headless 1
"""
...
def ensure_year_section(page, year: int, max_rounds=30):
    """
    兼容“同页分块”：往下滚，直到看到 “YYYY 年机型”
    """
...
def extract_summary_kv(page):
    """
    详情页上方摘要 KV（电视等级 / 游戏电视 / ALLM / VRR / 输入延时等）
    """
    js = r"""
    () => {
      function n(s){ return (s||'').replace(/\s+/g,' ').trim(); }
...
def main():
    ...
    ap.add_argument("--target_year", type=int, default=2026)      # ✅ 默认改为 2026
    ap.add_argument("--out_root", default="out_step3_2026")       # ✅ 默认改为 out_step3_2026
